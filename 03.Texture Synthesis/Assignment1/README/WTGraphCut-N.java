...
//			seh = (int) Math.ceil( tmp / ( samples[0].getWidth() ) /2) * 2;
			...
			for ( row = 2; row < seh; row+=2 ) {
				...
				for ( int col = 1; col < sew - 1; col+=2 ) {
					...
					nesw  = seset[row-1][col].first().tile.getNESW();
					x = nesw[x];
					y = seset[row-1][col+1].first().getDirection();
					nesw = seset[row-1][col+1].first().tile.getNESW();
					y = nesw[y];
				...
				//再合成 1 7 8 ,6 11 12
				int col = 0;
				y = seset[row][col+1].first().getDirection();
				nesw = seset[row-1][col].first().tile.getNESW();
				y = nesw[y];
				...
				col = sew - 1;
				x = seset[row][col-1].first().getDirection();
				nesw = seset[row][col-1].first().tile.getNESW();
				x = nesw[x];
				...
				for ( col = 2 ; col < sew - 2; col+=2) {
					x = seset[row][col].first().getDirection();
					nesw = seset[row][col].first().tile.getNESW();
					x = nesw[x];
					y = seset[row][col+1].first().getDirection();
					nesw = seset[row][col+1].first().tile.getNESW();
					y = nesw[y];
...
//			return e.tile.getStitchImg(e.getDirection() );
			return e.tile.getImage();
		}
		else {
			BufferedImage eimg = new BufferedImage( img.getWidth() ,img.getHeight() ,img.getType() );
	...
	public JPanel drawSynthesisTilesResult() {
		...
		for ( int i = 0; i < seh; i+=2 )
			for ( int j = 0; j < sew; j+=2 ) {
				JPanel p = getTileInfoPanel(seset[i][j].first().tile);
				panel.add(p);
			}
