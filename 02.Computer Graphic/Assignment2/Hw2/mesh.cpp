...
void mesh::LoadMesh(string obj_file)
{
	...
	while(!feof(scene))
	{
		...
		else if (!strcmp(token,"v"))//讀入點的座標
		{
			fscanf(scene,"%f %f %f",&vec[0],&vec[1],&vec[2]);
			vList.push_back(Vec3(vec));
		}

		else if (!strcmp(token,"vn"))//讀入點的normal
		{
			fscanf(scene,"%f %f %f",&vec[0],&vec[1],&vec[2]);
			nList.push_back(Vec3(vec));
		}
		else if (!strcmp(token,"vt"))//讀入點的texture coordinate
		{
			fscanf(scene,"%f %f",&vec[0],&vec[1]);
			tList.push_back(Vec3(vec));
		}

		else if (!strcmp(token,"f"))//讀入face f vertex_index/texture_index/normal_index
		{
			int i;
...
void mesh::Init(const char* obj_file)
{
	...
	setObjDirectory(obj_file);
	LoadMesh(string(obj_file));		// 讀入 .obj 檔 (可處理 Material)
}
void mesh::setObjDirectory( const char* obj_file ) {
	
	string str = obj_file;
	size_t dirindex =str.find_last_of("/");
	string dir = str.substr(0 ,dirindex );
	dir +="/";
	strcpy(obj_database ,dir.c_str() );
}
