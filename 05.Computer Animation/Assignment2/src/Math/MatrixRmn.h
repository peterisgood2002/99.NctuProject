...
public:
	...
	bool Solve( const VectorRn& b, VectorRn* x ) const;	  // Solves the equation   (*this)*x = b;    Uses row operations.  Assumes *this is invertible.   
	...
	bool ConvertToRefNoFree();				// Converts the matrix in place to row echelon form -- assumption is no free variables will be found
...
public:
	friend ostream& operator<<( ostream& o , const MatrixRmn& m );
};
/*Debug¥Î*/
ostream& operator<<( ostream& o , const MatrixRmn& m );
