...
void matrix_mult(const double a[][4], const double b[][4], double c[][4])
{
  int i, j, k;
  double tmp[4][4] = { 0 };
...
void matrix_identity( double a[][4] ) {

	a[0][0] = 1;	a[0][1] = 0;	a[0][2] = 0;	a[0][3] = 0;
	a[1][0] = 0;	a[1][1] = 1;	a[1][2] = 0;	a[1][3] = 0;
	a[2][0] = 0;	a[2][0] = 0;	a[2][2] = 1;	a[2][3] = 0;
...
//	double sin_theta = 
	...
	double r_axis[3], mag, theta;
	r_axis = {
	double mag = v3_mag(r_axis);
	if(mag > 1.) mag=1;         // hack to prevent error in asin() calculation caused by
	if(mag < -1.) mag=-1;       // the insufficient machine precision of doubleing point calculation
	theta=asin(mag);
