...
void CPhys::SetWorldParticles(tVector *coords,int particleCnt)
{
	...
	*/
	...
	ParticleSys[2] = (tParticle *)malloc(sizeof(tParticle) * particleCnt);
	for (int i = 0; i < 5; i++)
	{
	...
	memcpy(ParticleSys[2],CurrentSys,sizeof(tParticle) * particleCnt);

	ParticleSys[0] = CurrentSys;
	ParticleSys[1] = TargetSys;
}
////// SetWorldParticles //////////////////////////////////////////////////////
...
void CPhys::LoadData(FILE *fp)
{
	...
	fread(ParticleSys[0],sizeof(tParticle),ParticleCnt,fp);
	fread(ParticleSys[1],sizeof(tParticle),ParticleCnt,fp);
	fread(ParticleSys[2],sizeof(tParticle),ParticleCnt,fp);
...
void CPhys::SaveData(FILE *fp)
{
	...
	fwrite(ParticleSys[0],sizeof(tParticle),ParticleCnt,fp);
	fwrite(ParticleSys[1],sizeof(tParticle),ParticleCnt,fp);
	fwrite(ParticleSys[2],sizeof(tParticle),ParticleCnt,fp);
...
{
	memcpy(CurrentSys,ParticleSys[2],sizeof(tParticle) * ParticleCnt);
	memcpy(TargetSys,ParticleSys[2],sizeof(tParticle) * ParticleCnt);
}
...
void CPhys::SetVertexMass()
{
	...
	{
		ParticleSys[0][Pick[0]].oneOverM = dialog.m_VertexMass;
		ParticleSys[0][Pick[1]].oneOverM = dialog.m_VertexMass;
		ParticleSys[1][Pick[0]].oneOverM = dialog.m_VertexMass;
		ParticleSys[1][Pick[1]].oneOverM = dialog.m_VertexMass;
		ParticleSys[2][Pick[0]].oneOverM = dialog.m_VertexMass;
		ParticleSys[2][Pick[1]].oneOverM = dialog.m_VertexMass;
	}
}
...
void CPhys::ComputeForces( tParticle	*system, BOOL duringIntegration )
{
	...
	for (loop = 0; loop < SpringCnt; loop++)
	{
		...
		*/
		...
		/*加入Damping Force*/
		curParticle->f.x += Dterm * deltaP.x;
		curParticle->f.y += Dterm * deltaP.y;
		curParticle->f.z += Dterm * deltaP.z;
...
///////////////////////////////////////////////////////////////////////////////
void CPhys::IntegrateSysOverTime(tParticle *initial,tParticle *source, tParticle *target, float deltaTime)
{
	/*
	     .
	     X = f(X ,t)
