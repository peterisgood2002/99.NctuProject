...
// Purpose:		Actually breaks a string of words into individual pieces 根據空白切開字串
...
///////////////////////////////////////////////////////////////////////////////
	...
	in.TrimLeft();//如果前面有空白就忽略
	in.TrimRight();//如果最後有空白就忽略
	*cnt = 0;
	do 
	{
		...
		words->Add(temp);
		if (temp == in) break;//如果發現一樣 則表示他後面沒有在有新的字串 例如：TTT 
...
///////////////////////////////////////////////////////////////////////////////
	...
	for (loop = 1; loop < loopcnt; loop++)
	{
		...
		face->v[loop - 1] = atoi(vStr);		// STORE OFF THE INDEX FOR THE VERTEX 儲存面的Vertex
		face->t[loop - 1] = atoi(tStr);		// STORE OFF THE INDEX FOR THE TEXTURE 儲存面的Texture Coordinate
		face->n[loop - 1] = atoi(nStr);		// STORE OFF THE INDEX FOR THE NORMAL 儲存面的Normal
	}
	face->flags = 0;
...
/// Local Variables ///////////////////////////////////////////////////////////
	int loop,loop2,cnt;
	char buffer[MAX_STRINGLENGTH];//最大Buffer量為255
	CStringArray words;//將一行拆開成一堆字串
	CString temp;
	FILE *fp;
	long vCnt = 0, nCnt = 0, tCnt = 0, fCnt = 0;
	long vPos = 0, nPos = 0, tPos = 0, fPos = 0;
	tVector *vertex = NULL,*normal = NULL,*texture = NULL;//儲存OBJ檔所有的資料
	t_faceIndex *face = NULL;//表示為一個面集合
	float *data;
...
///////////////////////////////////////////////////////////////////////////////
	...
	if (fp != NULL)
	{
		// FIRST PASS SETS UP THE NUMBER OF OBJECTS IN THE FILE 1.找到有幾個點和面
		while (!feof(fp))
		{
			fgets(buffer,MAX_STRINGLENGTH,fp);	// GET A STRING FROM THE FILE
			ParseString(buffer,&words,&cnt);	// BREAK THE STRING INTO cnt WORDS
		...
		// NOW THAT I KNOW HOW MANY, ALLOCATE ROOM FOR IT 有了點面個數 就可以給定空間
		if (vCnt > 0)
		{
			...
			// NOW THAT IT IS ALL ALLOC'ED.  GRAB THE REAL DATA 取資料
			while (!feof(fp))
			{
				...
				if (cnt > 0)
				{
					...
					if (temp.GetLength() > 0)
					{
						...
						else if (temp[0] == 'f')			// f v/t/n v/t/n v/t/n	FACE LINE
						{
							...
							HandleFace(&words,&face[fPos]);//將面讀入
							fPos++;
						}
						else if (temp == "mtllib")  // HANDLE THE MATERIAL LIBRARY
						{
							LoadMaterialLib(words.GetAt(1),visual);//將Material的資訊讀入 並放置在t_Visual中
						}
					}
				}
				words.RemoveAll();		// CLEAR WORD BUFFER
			...
			if ((face[0].flags & FACE_TYPE_TRI)> 0) visual->vPerFace = 3;//判斷是三角形成一個面或是四邊形成一個面
			else visual->vPerFace = 4;

			if (nCnt > 0 && (flags & LOADOBJ_VERTEXONLY) == 0)//有Normal
			{
				if (tCnt > 0)//有Texture coordinate
				{
					visual->dataFormat = GL_T2F_N3F_V3F;
					visual->vSize = 8;					// 2 texture, 3 normal, 3 vertex
				}
				else//無Texture coordinate
				{
			...
			if ((flags & LOADOBJ_REUSEVERTICES) > 0)
			{
				...
				visual->vertexData = (float *)malloc(sizeof(float) * visual->vSize * vPos);//Vertex全部大小
				visual->vertexCnt = vPos;//設定Vertex數量
				visual->faceIndex = (unsigned short *)malloc(sizeof(unsigned short) * fPos * visual->vPerFace);//面的全部大小
				if ((flags & LOADOBJ_VERTEXONLY) > 0)		// COPY VERTEX DATA
				{
					memcpy(visual->vertexData,vertex,sizeof(float) * visual->vSize * vPos);//將點進行Copy
				}
